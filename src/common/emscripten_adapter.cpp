...
extern "C" {
    ...
    int openAdapter()
    {
        /*std::unique_ptr<std::thread> t(new std::thread(openAdapterAsync, [&](){
            (*emscriptenDebugOut)(createEmscriptenDebugText("Test"));
        }));*/
        std::unique_ptr<std::thread> t(new std::thread(openAdapterAsync));
        adapterThreads.push_back(std::move(t));
        (*emscriptenDebugOut)(createEmscriptenDebugText("Created a new adapter thread"));
        return 0;
    }
}